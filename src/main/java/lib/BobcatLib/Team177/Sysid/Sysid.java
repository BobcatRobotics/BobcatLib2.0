package lib.BobcatLib.Team177.Sysid;
...
public class Sysid extends SubsystemBase {
  ...
  public Sysid(SysidCompatibleSwerve swerve) {
    ...
    routine =
        new SysIdRoutine(
            ...
            new SysIdRoutine.Mechanism((swerve::sysidVoltage), this::logMotors, this));
  }
  ...
  private void logMotors(SysIdRoutineLog log) {
    log.motor("front-left")
        .voltage(
            m_appliedVoltage.mut_replace(
                swerve.getModuleVoltage(0) * RobotController.getBatteryVoltage(), Volts))
    ...
    log.motor("front-right")
        .voltage(
            m_appliedVoltage.mut_replace(
                swerve.getModuleVoltage(1) * RobotController.getBatteryVoltage(), Volts))
    ...
    log.motor("back-left")
        .voltage(
            m_appliedVoltage.mut_replace(
                swerve.getModuleVoltage(2) * RobotController.getBatteryVoltage(), Volts))
    ...
    log.motor("back-right")
        .voltage(
            m_appliedVoltage.mut_replace(
                swerve.getModuleVoltage(3) * RobotController.getBatteryVoltage(), Volts))
